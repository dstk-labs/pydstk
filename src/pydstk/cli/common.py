...
from pydstk.cli import cli_types
    ...
    def handle_parse_result(self, ctx, opts, args):
        ...
        if self.should_read_value_from_file(opts, args, config_file):
            with open(config_file) as f:
                ...
                if (
                    isinstance(self, DstkObjectListOption)
                    and self.object_list_name in config_data
                ):
                    value_list = []
                    ...
                    if value is not None:
                        if isinstance(value, dict):
                            ...
                            value = (value,)
                        # yaml.load turns datetime strings into datetime instances
                        # so we turn it back to a string
                        elif isinstance(value, datetime.datetime):
                            value = value.strftime("%Y-%m-%dT%H:%M:%S")

                        opts[self.name] = value

        rv = super(ReadValueFromConfigFile, self).handle_parse_result(ctx, opts, args)
        return rv
...
class DstkArgument(
    ColorExtrasInCommandHelpMixin, ReadValueFromConfigFile, click.Argument
):
    def should_read_value_from_file(self, opts, args, config_file):
        return opts.get(self.name) in (None, ()) and config_file


class DstkOption(ColorExtrasInCommandHelpMixin, ReadValueFromConfigFile, click.Option):
    def should_read_value_from_file(self, opts, args, config_file):
...
def generate_options_template(ctx, param, value):
    ...
    for param in ctx.command.params:
        ...
        if isinstance(param, DstkObjectListOption):
...
class DstkObjectListOption(DstkOption):
    def __init__(self, object_name, param_decls, **kwargs):
        super(DstkObjectListOption, self).__init__(param_decls, **kwargs)
...
def validate_comma_split_option(
    comma_option_value, option_value, raise_if_no_values=False
):
